  ...
  def step(): Try[Int] =
    Try {
      ...
      if (newConfirmedHeight > lastSeenHeight) {
        ...
        val (from, to) = RangeUtil.fromToSeq(_from, _to, by).head

        val newBlocks = Await.result(
          api.blocksByHeightRange(from, to), 1.minute)
        logger.debug(s"Loaded part ($from - $to: ${newBlocks.size}) of $network blocks ${_from} - ${_to} with ids: ${newBlocks.map(_.signature).mkString(",")}")
